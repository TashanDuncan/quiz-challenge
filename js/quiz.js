...
// Loop through Array, create list item which stores each value in a li tag
const createList = (arr) => {
  const newList = [];
  for(let i = 0; i < arr.length; i++) {
    newList.push(`<li>${arr[i]}</li>`);
  }
  return newList.join("");
}
...
const rightList = createList(rightQuestions);
const wrongList = createList(wrongQuestions);
